...
\begin{document}
	...
	\subsection*{Flag complexes:} \label{flag complexes}
	
	Let $G$ be a $1$-dimensional abstract (resp. directed) simplicial complex. The complex $\langle G \rangle$ has the same set of vertices as $G$ and $\{v_0, \dots, v_n\}$ \big(resp. $(v_0, \dots, v_n)$\big) is a simplex in $\langle G \rangle$ if an only if $\{v_i, v_j\}$ \big(resp. $(v_i, v_j)$\big) is in $G$ for each pair of vertices $v_i, v_j$. 
	
	A (directed) simplicial complex $X$ is a \textit{(directed) flag complex} if $X = \langle G \rangle$ for some $G$.
	
	Given a collection of weights in $G$, i.e., a function
	\begin{equation*}
	w : G \to \mathbb R,
	\end{equation*}
	consider the extension
	\begin{equation*}
	w : \langle G \rangle \to \mathbb R
	\end{equation*}
	given by
	\begin{equation*}
	...
	A filtered complex $\{X_s\}_{s \in \mathbb R}$ is a \textit{(directed) filtered flag complex} if $X_s = \langle G \rangle_s$ for some $(G,w)$.
