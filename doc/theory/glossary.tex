...
\begin{document}
	...
	\end{enumerate}
	Every simplicial complex defines an
	...
	Let $(X, d)$ be a 
	% \hyperref[finite_metric_spaces_and_point_clouds]{point cloud}
	...
	The set of persistence diagrams together with any of the distances above is a \hyperref[metric_space]{metric space}.
  ...
  % \hyperref[vectorization_kernel_amplitude]{vectorization}
  ...
  $p$-integrable, that is to say,
	\begin{equation} \label{equation_persistence_landscape_norm}
	...
	\end{equation*}
	is finite. In this case we refer to (\ref{equation_persistence_landscape_norm}) as the
  % \hyperref[vectorization_kernel_amplitude]{amplitude}
  amplitude of the persistence landscape of $D$.
  ...
  vectorization of the set of persistence diagrams with target the vector space of continuous real-valued functions on $\mathbb R$.
	...
	\subsection*{Metric space} \label{metric_space}
		...
		\end{equation*}
		is said to be a \textit{metric space} if the values of $d$ are all non-negative and if, for all $x,y,z \in X$,
		\begin{equation*}
	...
	\subsection*{Inner product and norm} \label{inner_product_norm}
	...
	||u+v|| = ||u|| + ||v||.
	\end{equation*}
	In this case the function $||-||$ is referred to as the \textit{norm}. An inner product space is naturally a normed space with
	\begin{equation*}
	||u|| = \sqrt{\langle u, u \rangle}
	\end{equation*}
	and a normed space is naturally a \hyperref[metric_space]{metric space} with distance function
	\begin{equation*}
	d(u,v) = ||u-v||.
	\end{equation*}
  ...
  % \hyperref[inner_product_norm]{inner product space}
  inner product space with inner product 
	...
	This inner product is referred to as \textit{dot product} and the associated norm and distance function are respectively named \textit{euclidean norm} and \textit{euclidean distance}.
	 ...
	 \subsection*{$L^p$-norm} \label{lp_norm}
	 
	 ...
	
	\subsection*{Vectorization, kernel and amplitude} \label{vectorization_kernel_amplitude}
  ...
  % \hyperref[inner_product_norm]{normed space}
