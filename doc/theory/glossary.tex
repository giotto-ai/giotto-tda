...
\begin{document}
  ...
  Let ${(b_i, d_i)}{i \in I}$ be a \hyperref[persistence diagram] {persistence diagram}. Its \textit{associated persistence landscape} $\lambda$ is defined by letting $\lambda_k(t)$ be the $k$-th largest value of the collection ${ \Lambda_i(t) }{i \in I}$, where
	...
	\subsection*{Heat kernel:} \label{heat kernel}
	
	Let $(\xi, k)$ be a pair with $\xi$ a set and
	\begin{equation*}
		k:\xi \times \xi \rightarrow \mathbb{R}
	\end{equation*}
	a function.	 A \textit{kernel} for this pair is a function
	\begin{equation*}
		\phi: \xi \rightarrow H
	\end{equation*}
	where $\big( H, \langle -, - \rangle \big)$ is an \hyperref[inner product and 			metric spaces]{inner product space} and $\phi$ satisfies
	\begin{equation*}
		k(x,y) = \big\langle \phi(x), \phi(y) \big\rangle.
	\end{equation*}
	Let $\xi$ be the set of \hyperref[persistence diagram]{persistence diagrams} 	and $\sigma$ a positive real number. Denoting
	\begin{equation*}
		\Omega = \big\{ (x_1,x_2) \in \mathbb R^2\ |\ x_1 \leq x_2 \big\}
	\end{equation*}
	consider the inner product space of square integrable function $L^2(\Omega)$ and the map $\phi_\sigma : \xi \to L^2(\Omega)$ described next. Given a persistence diagram $D$, the function $\phi_\sigma(D)$ is the time $\sigma$ solution to the heat equation with initial condition specified by the sum of Dirac delta functions concentrated over the off diagonal points of $D$. More explicitly, the function $\phi_\sigma(D)$ is the time $\sigma$ solution to
	\begin{align*}
	...
	\end{align*}
	The \textit{heat kernel} is defined by the function $\phi_\sigma$ on the set of 		persistence diagrams together with the function
	\begin{equation*}
		k_\sigma(D,\, D') = \frac{1}{8\pi\sigma}\sum_{p\in D,\, q \in D'}
		\left(
		\exp\left(\frac{-\Vert p-q\Vert^2}{8\sigma}\right) -
		\exp\left(\frac{-\Vert p-\bar{q}\Vert^2}{8\sigma}\right)
		\right)
	\end{equation*}
	where $\bar{q} = (d,b)$ for a point $q=(b,d)$.

	\paragraph{\\ References:} \cite{reininghaus2014stable}
