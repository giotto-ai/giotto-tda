...
\begin{document}
	...
	\end{equation*}	
	is finite. In this case we refer to (\ref{equation:persistence_landscape_norm}) as the $p$-\textit{landscape norm} of $D$ and, for $p = 2$, define the value of the \textit{landscape kernel} on two persistence diagrams $D$ and $E$ as
	\begin{equation*}
	...
	\begin{align} \label{equation:heat_equation}
	...
	\end{align}
	where $\Omega = \{(x_1, x_2) \in \mathbb R^2\ |\ x_1 \leq x_2\}$, then solving the same equation after precomposing the data of (\ref{equation:heat_equation}) with the change of coordinates $(x_1, x_2) \mapsto (x_2, x_1)$, and defining the image of $D$ to be the difference between these two solutions at the chosen time $t$.
	
	Similarly, the \textit{rotation heat vectorization} is defined by sending $D$ to the solution, evaluated at time $t$, of the equation obtained by precomposing the data of (\ref{equation:heat_equation}) with the change of coordinates $(x_1, x_2) \mapsto (x_1, x_2-x_1)$.
