...
\begin{document}
  ...
  Let ${(b_i, d_i)}{i \in I}$ be a \hyperref[persistence diagram] {persistence diagram}. Its \textit{associated persistence landscape} $\lambda$ is defined by letting $\lambda_k(t)$ be the $k$-th largest value of the collection ${ \Lambda_i(t) }{i \in I}$, where
	...
	Given a set $\xi$, $k:\xi \times \xi \rightarrow \mathbb{R}$ is a kernel, if there exists a Hilbert space $H$ with inner product $\langle \cdot, \cdot \rangle$ and a function $\phi: \xi \rightarrow H$, such that $k(x,y) = \langle \phi(x), \phi(y) \rangle$.
