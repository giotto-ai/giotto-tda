...
\begin{document}
	...
	\begin{tabular}{ l l l}
		...
		$\Delta$ & : & The \nameref{multiset} $\{(s, s) \, : \ s \in \mathbb R\}$ with multiplicity $(s,s) \mapsto +\infty$.
	\end{tabular}
	...
	A \textit{time series} is a sequence $\{y_t\}_{t = 0}^n$ of real numbers, indexed by a discrete ``time'' parameter.
	
	A common construction of a times series $\{x_t\}_{t = 0}^n$ is given by choosing $x_0$ arbitrarily as well as a step parameter $h$, and setting
	\begin{equation*}
	x_t = x_0 + h\cdot t.
	\end{equation*}
	Another usual construction is as follows: given a time series $\{x_t\}_{t = 0}^n \subseteq U$ and a function
	\begin{equation*}
	f : U \subseteq \mathbb R \to \mathbb R,
	\end{equation*}
	we obtain a new time series $\{f(x_t)\}_{t = 0.}^n$
	
	Generalizing the previous construction we can define a time series from a function
	\begin{equation*}
	\varphi : \{0, \ldots, n\} \time M \to M, \qquad M \subseteq \mathbb R^d
	\end{equation*} 
	using a function $f : M \to \mathbb R$ and an arbitrarily chosen $m \in M$. This is
	\begin{equation*}
	\{y^{(m)}_t\}_{t = 0}^n = \{ f(\varphi(t, m)) \}_{t=0}^n.
	\end{equation*}
