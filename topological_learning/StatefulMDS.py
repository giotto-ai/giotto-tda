...
def _smacof_single(dissimilarities, X_previous = None, metric=True, n_components=2, init=None,
    ...
    """
    dissimilarities = check_symmetric(dissimilarities, raise_exception=False)
    ...
    for it in range(max_iter):
        ...
        if X_previous is not None:
            X[:X_previous.shape[0], :] = X_previous
    ...
    if effective_n_jobs(n_jobs) == 1:
        for it in range(n_init):
            pos, stress, n_iter_ = _smacof_single(
                dissimilarities, X_previous=X_previous, metric=metric,
        ...
        results = Parallel(n_jobs=n_jobs, verbose=max(verbose - 1, 0))(
            delayed(_smacof_single)(
                dissimilarities, X_previous=X_previous, metric=metric, n_components=n_components,
...
class StatefulMDS(BaseEstimator, TransformerMixin):
    ...
    def fit(self, XList, y=None, init=None):
        """
        ...
        if self.dissimilarity == 'precomputed':
            self.dissimilarity_matrix_fit_ = X
        elif self.dissimilarity == 'euclidean':
            self.dissimilarity_matrix_fit_ = euclidean_distances(X)
        else:
    ...
    def transform(self, XList, y=None, init=None):
        """
        ...
        if np.array_equal(X, self.input_fit_):
            XTransformed = self.embedding_fit_
            ...
            if self.dissimilarity == 'precomputed':
                ...
                self.dissimilarity_matrix_[:self.n_samples_fit, :self.n_samples_fit] = self.dissimilarity_matrix_fit_
                self.dissimilarity_matrix_[:, self.n_samples_fit:] = X
                self.dissimilarity_matrix_ = np.maximum(self.dissimilarity_matrix_, self.dissimilarity_matrix_.T)

            elif self.dissimilarity == 'euclidean':
                self.dissimilarity_matrix_ = euclidean_distances(np.hstack([self.embedding_fit_, X]))
            else:
                raise ValueError("Proximity must be 'precomputed' or 'euclidean'."
                                 " Got %s instead" % str(self.dissimilarity))
            ...
            XTransformed = self.embedding_[self.n_samples_fit:, :]
