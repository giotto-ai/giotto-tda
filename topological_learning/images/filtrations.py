...
class ImageInverter(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        ...
        X_transformed = Parallel(n_jobs=self.n_jobs) ( delayed(np.logical_not) (X[slice_indices[i] : slice_indices[i+1]])
...
class HeightFiltration(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, direction=None, normalize=False, len_vector=8, n_jobs=None):
    ...
    def get_params(self, deep=True):
        ...
        params : mapping of string to any
            Parameter names mapped to their values.
        """
        return {'direction': self.direction, 'normalize': self.normalize,
    ...
    def fit(self, X, y=None):
        ...
        axis_order = [2, 1, 3]
        mesh_range_list = [ np.arange(X.shape[order]) if self._direction[i] >= 0
                            else -np.flip(np.arange(X.shape[order]))
                            for i, order in enumerate(axis_order[:self._dimension])]

        self._mesh = np.stack(np.meshgrid(*mesh_range_list, indexing='xy'), axis=self._dimension)
...
class ThickeningFiltration(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, n_iterations=None, normalize=False, len_vector=8, n_jobs=None):
    ...
    def get_params(self, deep=True):
        ...
        params : mapping of string to any
            Parameter names mapped to their values.
        """
        return {'n_iterations': self.n_iterations, 'normalize': self.normalize,
                'len_vector': self.len_vector, 'n_jobs': self.n_jobs}
    ...
    def _calculate_filtration(self, X):
        ...
        for iteration in range(1, self.n_iterations + 1):
            X_dilated = np.asarray([ndi.binary_dilation(X_thick[i])  for i in range(X_thick.shape[0])])
            X_thick += (X_thick + X_dilated == 1) * X_dilated * (iteration + 1)
    ...
    def transform(self, X, y=None):
        ...
        if self.normalize:
            norm = np.linalg.norm(X.shape[1:], ord=np.inf)
            X_transformed /= norm
...
class RadialFiltration(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, center=None, max_radius=np.inf, metric='euclidean', metric_params={},
                 normalize = False, len_vector=8, n_jobs=None):
        self.center = center
    ...
    def get_params(self, deep=True):
        ...
        return {'center': self.center,'max_radius': self.max_radius,
                ...
                'normalize': self.normalize, 'len_vector': self.len_vector,
                'n_jobs': self.n_jobs}
    ...
    def _calculate_filtration(self, X):
        ...
        X_rad[np.logical_or(X_rad == -1, X == False)] = np.inf
        X_rad = X_rad / norm
        return X_rad
    ...
    def fit(self, X, y=None):
        ...
        axis_order = [2, 1, 3]
        mesh_range_list = [ np.arange(0, X.shape[i]) for i in axis_order[:self._dimension] ]
...
class DensityFiltration(BaseEstimator, TransformerMixin):
    ...
    def _calculate_filtration(self, X):
        X_dense = np.logical_not(X)*1.

        for iteration in range(1, self.n_iterations + 1):
            X_dilated = np.asarray([ndi.binary_dilation(X_dense[i], border_value=0)  for i in range(X_dense.shape[0])])
            X_dense += (X_dense + X_dilated == 1) * X_dilated * (iteration + 1)

        mask_filtered = X_dense == 0
        X_dense -= 1
        X_dense[mask_filtered] = np.inf
        return X_dense
...
class SignedDistanceFiltration(BaseEstimator, TransformerMixin):
    ...
    def _calculate_filtration(self, X):
        ...
        for iteration in range(self.n_iterations + 1, 2 * self.n_iterations + 1):
            X_dilated = np.asarray([ndi.binary_dilation(X_dense[i], border_value=0)  for i in range(X_dense.shape[0])])
            X_dense += (X_dense + X_dilated == 1) * X_dilated * (iteration + 1)

        mask_filtered = np.logical_or(X_thick == 0, X_dense == 0)
        X_thick += 1
        X_dense -= 1
        X_thick[mask_filtered] = np.inf
        X_dense[mask_filtered] = np.inf
        return (X_thick + X_dense) / 2
