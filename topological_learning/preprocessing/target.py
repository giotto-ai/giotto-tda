...
class Labeller(BaseEstimator, TransformerMixin):
    ...
    def get_params(self, deep=True):
        return {'labelling_kwargs': self.labelling_kwargs, 'function_kwargs': self.function_kwargs,
                'window_size': self.window_size, 'percentiles': self.percentiles,
    ...
    def _embed(y, window_size):
        ...
        y = np.flip(y)
        y_embedded = np.stack( [ y[i: i + window_size].flatten() for i in range(0, n_windows) ])
    ...
    def fit(self, y, X=None):
        ...
        self._y_transformed = Labeller.implementedLabellingRecipes[labelling_kwargs.pop('type')](function_kwargs.pop('function'), self._y, **labelling_kwargs, **function_kwargs)

        if self.percentiles is not None:
            self.thresholds = [np.percentile(np.abs(self._y_transformed.flatten()), percentile) for percentile in self.percentiles]
        else:
    ...
    def transform(self, y):
        ...
        if np.array_equal(y, self._y):
            ...
            labelling_kwargs = self.labelling_kwargs.copy()
            function_kwargs = self.function_kwargs.copy()
            y_transformed = Labeller.implementedLabellingRecipes[labelling_kwargs.pop('type')](function_kwargs.pop('function'), y, **labelling_kwargs, **function_kwargs)

        if self.thresholds is not None:
            y_transformedAbs = np.abs(y_transformed)
            y_transformed = np.concatenate([1 * (y_transformedAbs >= 0) * (y_transformedAbs < self.thresholds[0])] +\
                                          [1 * (y_transformedAbs >= self.thresholds[i])
                                           * (y_transformedAbs < self.thresholds[i+1])
                                           for i in range(len(self.thresholds)-1)] +\
                                          [1 * (y_transformedAbs >= self.thresholds[-1])], axis=1)
            y_transformed = np.nonzero(y_transformed)[1].reshape((y.shape[0], 1))
