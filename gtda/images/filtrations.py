...
class SignedDistanceFiltration(BaseEstimator, TransformerMixin):
    ...
    def _calculate_dilation(self, X):
        X_dilation = X * 1.
        X_erosion = np.logical_not(X) * 1.
        mask = X == 1

        for iteration in range(1, self.n_iterations_ + 1):
            X_dilated = np.asarray([ndi.binary_dilation(X_dilation[i]) for i in range(X_dilation.shape[0])])
            X_dilation -= (X_dilation + X_dilated == 1) * X_dilated * (iteration)

        for iteration in range(0, self.n_iterations_ + 1):
            X_eroded = np.asarray([ndi.binary_dilation(X_erosion[i], border_value=0) for i in range(X_erosion.shape[0])])
            X_erosion += (X_erosion + X_eroded == 1) * X_eroded * (iteration)

        mask_erosion = X_erosion == 0
        mask_dilation = X_dilation == 0
        X_erosion[np.logical_not(mask)] = 0
        X_erosion[mask] -= 1
        X_dilation[mask] = 0
        X_dilation[mask_dilation] = -self.max_value_
        X_erosion[mask_erosion] = self.max_value_
        return (X_dilation + X_erosion)
