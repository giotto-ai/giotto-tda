...
def get_sparse_distance_matrices(draw):
    ...
    shape_kwargs = {"shape": (0, 0)} if not_inf_idx.any() else {}
...
def test_coo_below_diagonal_and_mixed_same_as_above(distance_matrix):
    ...
    kwargs = {"maxdim": 2, "metric": "precomputed"}

    pd_above = ripser(distance_matrix, **kwargs)['dgms']

    pd_below = ripser(distance_matrix.T, **kwargs)['dgms']

    _row, _col, _data = (distance_matrix.row, distance_matrix.col,
                         distance_matrix.data)
    to_transpose_mask = np.full(len(_row), False)
    to_transpose_mask[np.random.choice(range(len(_row)), size=len(_row) // 2,
                                       replace=False)] = True
    ...
    distance_matrix_mixed = coo_matrix((_data, (row, col)))
    pd_mixed = ripser(distance_matrix_mixed, **kwargs)['dgms']
    ...
    pd_conflicts = ripser(distance_matrix_conflicts, **kwargs)['dgms']

    for i in range(kwargs["maxdim"] + 1):
