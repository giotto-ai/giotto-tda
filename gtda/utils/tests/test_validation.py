...
from gtda.utils.validation import check_diagrams, validate_params, \
    check_point_clouds, check_multi_time_series
...
def test_check_multi_time_series_ragged_array():
    X = np.array([np.arange(2), np.arange(3)], dtype=object)
    with pytest.raises(ValueError):
        check_multi_time_series(X)


def test_check_multi_time_series_array_of_list():
    X = np.array([list(range(2)), list(range(3))])
    with pytest.raises(ValueError):
        check_multi_time_series(X)


def test_check_multi_time_series_list_of_list():
    X = [list(range(2)), list(range(3))]
    Xnew = check_multi_time_series(X)
