...
def _pad(X, max_betti_numbers):
    X_padded = {
        dimension:
            np.pad(X[dimension],
                   ((0, 0),
                    (0, max_betti_numbers[dimension] - X[dimension].shape[1]),
                    (0, 0)), 'constant') for dimension in X.keys()}
    return X_padded
...
def _sort(XScaled, homology_dimensions):
    ...
    indices = {
        dimension: np.stack([indices[dimension], indices[dimension]],
                            axis=2) for dimension in homology_dimensions}
    XSorted = {
        dimension: np.flip(np.take_along_axis(XScaled[dimension],
                                              indices[dimension],
                                              axis=1), axis=1)
        for dimension in homology_dimensions}
...
def _filter(XScaled, homology_dimensions, cutoff):
    ...
    maxPoints = {
        dimension: np.max(np.sum(X[:, :, 1] != 0,
                                 axis=1)) for dimension, X in XFiltered.items()
        }
