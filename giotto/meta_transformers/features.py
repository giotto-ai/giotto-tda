...
class EntropyGenerator(BaseEstimator, TransformerMixin):
    """Meta_transformer that returns the persistent entropy.
    ...
    scaler_metric : 'bottleneck' | 'wasserstein' | 'landscape' | 'betti', optional, default: 'bottleneck'
    ...
    def __init__(self, metric='euclidean', max_edge_length=np.inf,
                 ...
                 scaler_metric_params = None, delta = 0., n_jobs=None,
                 function=np.max):
        self.steps = None
        self.metric = metric
    ...
    def fit(self, X, y=None):
        ...
        X : ndarray, shape: (n_samples, n_points, n_dimensions)
            Input data. ``n_samples`` is the number of point clouds, ``n_points`` is the
            number of points per point clud and ``n_dimensions`` is the number
            of features for each point of the point cloud (i.e. the dimension
            of the point cloud space)
        ...
        validate_metric_params(self.scaler_metric, self.effective_metric_params_)

        
        if self.homology_dimensions is None:
            self.effective_homology_dimensions_ = [0, 1]
        else:
            self.effective_homology_dimensions_ = self.homology_dimensions.copy()

        self._validate_params()

        self._is_fitted = True
        
        self.steps = [('diagram', hl.VietorisRipsPersistence(metric=self.metric,
                                                             max_edge_length=self.max_edge_length,
                                                             homology_dimensions=self.effective_homology_dimensions_,
                                                             n_jobs=self.n_jobs)),
                 ('rescaler', diag.Scaler(metric=self.scaler_metric,
                                                 metric_params=self.effective_metric_params_,
                                                 function=self.function,
                                                 n_jobs=self.n_jobs)),
                 ('filter', diag.Filtering(delta=self.delta,
                                               homology_dimensions=self.effective_homology_dimensions_)),
...
class BettiCurveGenerator(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        """Extract the persistent entropy friom X.
            ...
            Returns
            -------
            X_transformed : ndarray, shape: (n_samples, n_homology_dimensions, n_sampled_values)
                ...
                used to sample the continous Betti Curves into arrays.
...
class LandscapeGenerator(BaseEstimator, TransformerMixin):
    """Meta_transformer that returns the sampled Persistence Landscape.
        ...
        n_layers : int, optional, default: 1
            Used to specify which of the persistence landscape profiles to consider.
    ...
    def fit(self, X, y=None):
        ...
        self.steps = [('diagram', hl.VietorisRipsPersistence(metric=self.metric,
                 ...
                 ('filter', diag.Filtering(delta=self.delta,
                                               homology_dimensions=self.effective_homology_dimensions_)),
                 ('landscape', diag.PersistenceLandscape(n_values = self.n_values,
    ...
    def transform(self, X, y=None):
        """Extract the persistent entropy from X.
            ...
            X_transformed : ndarray, shape: (n_samples, n_homology_dimensions, n_sampled_values)
                ...
                dimensions considered, i.e. the length of ``homology_dimensions``.
                The parameter ``n_sampled_values`` is the number of points
                used to sample the continous Persistence Landscape into arrays.
